...
# 4) 刺激--接入nest
if feats:
    onset_ms = aud_on + feats["latency_ms"]   # 对齐 NEST 声刺激时刻 aud_on
    # tau_ms   = feats["sigma_ms"]
    tau_ms = max(10.0, min(20.0, feats["sigma_ms"]))

    # S        = 0.0005 * feats["A_TC"]           # 简单缩放；后续再调标度
    # S的新生成方式
    amp_cap = 10.0
    # 温和增益，从很小开始
    s_gain = 1e-4
    # 归一化+裁剪后的权重（0..s_gain）
    S = s_gain * max(0.0, min(1.0, feats["A_TC"] / (amp_cap + 1e-9)))

else:
    onset_ms, tau_ms, S = 10000.0, 50.0, 0.0
n_regions = 68
weight = [0.0] * n_regions
target_idx = 18           # 例如 A1 / auditory cortex 区域
weight[target_idx] = S

print(parameters.parameter_connection_between_region['path'])
parameters.parameter_stimulus['onset']    = onset_ms
parameters.parameter_stimulus['tau']      = tau_ms
